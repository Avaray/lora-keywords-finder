...
- The extension has been tested with [Forge Neo 2.29.0](https://github.com/Haoming02/sd-webui-forge-classic/tree/neo) ([Gradio 4.40.0](https://gradio.app/changelog#4-40-0)) on Windows.
...
- If the [CivitAI API](https://developer.civitai.com/site/reference/) goes down, new data cannot be fetched, but your previously cached data will be available. In advanced options, you can download data for all models at once.
- Prompts and keywords will only be returned if the author specified them during the model upload. Many authors forget to provide them and end up adding them later in the model description. In this situation, obtaining keywords is very difficult. However, the most likely reason a model has no keywords is that it doesn't need them.
